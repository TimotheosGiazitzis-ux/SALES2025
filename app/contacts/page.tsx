...
export default async function ContactsPage() {
  ...
  if (!user) {
    return (
      <main className="card">
        <p>Bitte <Link href="/login">einloggen</Link>.</p>
      </main>
    );
  }
  ...
  // Kontakte + Kunden + Aktionen (vereinfachtes View via Join)
  const { data, error } = await supabase
    .from("contacts")
    .select(`
      id, name, email, phone, title, notes,
      customer:customers ( id, name, street, zip, city, country ),
      flags:action_flags ( * )
    `)
