...
export default function ContactsTable({ role, rows }: { role: "admin" | "sales" | "azubi"; rows: any[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (actionFilter) {
        const f = r.flags?.[0];
        if (!f || !f[actionFilter]) return false;
      }
      return true;
    });
        ...
        <div className="kpi" style={{ flex: 1, minWidth: 280 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 6 }}>
            {ACTIONS.slice(0, 7).map((a) => (
              <span key={a.key} className="badge">{a.label}: {kpis.perAction[a.key]}</span>
            ))}
          </div>
        </div>
      </div>
              ...
              <tr key={r.id}>
                ...
                <td style={{ minWidth: 420 }}>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {ACTIONS.map((a) => (
                      <label key={a.key} className="badge" style={{ cursor: writable ? "pointer" : "not-allowed", opacity: writable ? 1 : 0.6 }}>
                        <input
                          type="checkbox"
                          checked={!!f[a.key]}
                          disabled={!writable}
